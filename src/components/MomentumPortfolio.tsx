...
const MomentumPortfolio = () => {
  ...
  // Determine which stocks to show based on authentication
  const displayedStocks = user ? currentQuarterData.stocks : currentQuarterData.stocks.slice(0, 5);
  const hiddenStocksCount = currentQuarterData.stocks.length - displayedStocks.length;
  ...
  const PerformanceChart = () => {
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-4 sm:p-6 mb-6 sm:mb-8">
        <h2 className="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6">Historical Performance Comparison</h2>
        ...
        <div className="overflow-x-auto">
          <div className="relative" style={{ minWidth: `${chartWidth}px`, height: `${chartHeight + 60}px` }}>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {displayedStocks.map((stock, index) => (
                  <tr key={stock.symbol} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 sm:px-6 py-4 whitespace-nowrap">
                      <div className={`flex items-center space-x-1 ${getReturnColor(stock.quarterlyReturn)}`}>
                ...
                {/* Blurred rows for unauthenticated users */}
                {!user && hiddenStocksCount > 0 && (
                  <>
                    {currentQuarterData.stocks.slice(5).map((stock, index) => (
                      <tr key={`blurred-${stock.symbol}`} className="relative">
                        <td className="px-3 sm:px-6 py-4 whitespace-nowrap text-sm text-gray-900 blur-sm">
                          {5 + index + 1}
                        </td>
                        <td className="px-3 sm:px-6 py-4 whitespace-nowrap blur-sm">
                          <div className="flex items-center space-x-2 sm:space-x-3">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full overflow-hidden bg-gray-100 flex items-center justify-center flex-shrink-0">
                    ...
                    {/* Unlock overlay */}
                    <tr>
                      <td colSpan={4} className="px-3 sm:px-6 py-8">
                        <div className="text-center">
                          <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-6 border-2 border-dashed border-blue-200">
                            ...
                            <h3 className="text-lg font-semibold text-gray-800 mb-2">
                              Unlock {hiddenStocksCount} More Holdings
                            </h3>
                            <p className="text-gray-600 mb-4">
                              Sign in to view the complete portfolio with detailed analytics and historical performance
                            </p>
                            <button
